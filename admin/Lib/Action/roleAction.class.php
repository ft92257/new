...
class roleAction extends baseAction
{
	...
	{   
		$access_mod=D('access');
		$role_id = intval($_REQUEST['id']);
		$access_mod->where("role_id=".$role_id)->delete();
		$node_ids = $_REQUEST['access_node'];
		
		foreach ($node_ids as $node_id) {
			//$model   = D('node')->field('id,module')->where('id='.$node_id)->find();
			//$actions = D("node")->field('id,module,auth_type,action')->where("data='' AND status = 1 and auth_type < 2 AND module='".$model['module']."'")->select();
			$data['role_id'] = $role_id;
			$data['node_id'] =$node_id ;//$val['id'];
			$access_mod->add($data);
			dump($access_mod->getlastsql());
		//	/*			
